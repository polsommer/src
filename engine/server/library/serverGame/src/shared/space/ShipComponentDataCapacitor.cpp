...
bool ShipComponentDataCapacitor::readDataFromComponent (TangibleObject const & component)
{
	...
	if (!objvars.getItem (Objvars::capacitorEnergyCurrent, m_capacitorEnergyCurrent))
		WARNING (false, ("ShipComponentDataCapacitor [%s] has no capacitorEnergyCurrent [%s]", component.getNetworkId ().getValueString ().c_str (), Objvars::capacitorEnergyCurrent.c_str ()));
	if (!objvars.getItem (Objvars::capacitorEnergyMaximum, m_capacitorEnergyMaximum))
		WARNING (false, ("ShipComponentDataCapacitor [%s] has no capacitorEnergyMaximum [%s]", component.getNetworkId ().getValueString ().c_str (), Objvars::capacitorEnergyMaximum.c_str ()));
	if (!objvars.getItem (Objvars::capacitorEnergyRechargeRate, m_capacitorEnergyRechargeRate))
		WARNING (false, ("ShipComponentDataCapacitor [%s] has no capacitorEnergyRechargeRate [%s]", component.getNetworkId ().getValueString ().c_str (), Objvars::capacitorEnergyRechargeRate.c_str ()));
